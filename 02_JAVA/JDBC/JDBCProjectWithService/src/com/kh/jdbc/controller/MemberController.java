...
public class MemberController {
	...
	public void bulkInsertMembers(int count) {
		Scanner sc = new Scanner(System.in);
		int successCount = 0;

		for (int i = 0; i < count; i++) {
			System.out.println((i + 1) + "번째 회원 정보를 입력하세요.");
		...
		if (successCount > 0) {
			new MemberMenu().displaySuccess(successCount + "명의 회원이 성공적으로 추가되었습니다.");
		} else {
			new MemberMenu().displayFail("회원 추가에 실패하였습니다.");
		}
	}

	public void bulkDeleteMembers(int count) {
		Scanner sc = new Scanner(System.in);
		int successCount = 0;
		...
		if (successCount > 0) {
			new MemberMenu().displaySuccess(successCount + "명의 회원이 성공적으로 삭제되었습니다.");
		} else {
