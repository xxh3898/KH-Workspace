...
public class ArrayMemory {
	...
	public static void main(String[] args) {
		...
		/*
		 * 실제 리터럴값을 곧바로 담을 수 있는 변수 일반변수
		 * 다른 메모리의 주소값을 담고있는 변수를 참조변수라고 함
		 * 
		 * 기본자료형(원시타입) : int, double, float, char, long, ...
		 * -> 실제 리터럴값을 바로 담을 수 있음
		 * 그외 자료형(String, Scanner, int[], double[], ...
		 * -> 참조타입(필요한 메모리의 크기가 가변적이기 떄문에
		 * */
		...
		// 배열의 가장 큰 단점
		// 배열은 한번 지정하면 크기 변경 불가
		System.out.println(arr.hashCode());
		arr=new int[10];
		System.out.println(arr.hashCode());
		// 주소값이 변경됨 -> 수정이 불가하여 새로운 메모리공간 할당
		
		/*
		 * 연결이 끊어진 기존 배열의 메모리공강는 Heap영역에 남아있다. (어떤 변수에도 참조되지 않는 상태)
		 * => 일정시간이 지나면 "GC(가비지 컬렉터)"가 알아서 회수한다.
		 * => 자바에서의 "자동메모리관리"특징 => 개발자는 코드에 지비중할 수 있음
		 * */
		
		arr=null;
		//배열을 더이상 사용하지 않을 떄 null을 대입하면 메모리를 반환
		
		int[] arr4= {1,1,1,1,1};
		int[] arr5= {1,1,1,1,1};
		
		System.out.println(arr4==arr5);
		// fasle -> 각 참조변수는 ==통해 비교시 주소값을 비교하기 때문에
