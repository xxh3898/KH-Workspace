/*
    <DCL : 데이터 제어문>
    계정에 시스템권환 또는 객체접근 권한을 부여하거나 회수하는 구문
    즉, 누가 어떤 객체에 무엇을 할 수 있는지를 통제함
    
    > 시스템권한 : DB객체생성, 사용자생성권한 등 관리자 권한
    > 객체접근권한 : 테이블, 뷰, 시퀀스 등 특정 객체에 접근 권환
...
/*
    ...
    DML문 한 개를 수행할 때 트랜잭션이 존재하지 않으면 트랜잭션을 만들어서 묶음
...
-- AUTOCOMMIT 여부 확인
SHOW AUTOCOMMIT;
-- AUTOCOMMIT 수동설정 / ON -> 자동으로 DML 실행시마다 COMMIT이 진행됨
SET AUTOCOMMIT OFF;
...
INSERT INTO EMP_01 VALUES(801, '김민규', '기술지원부');
INSERT INTO EMP_01 VALUES(802, '정수빈', '창업지원부');
ROLLBACK TO SP;
...
/*
    ...
    I(Isolation) 독립성 : 여러 트랜잭션이 동시에 실행돼도 서로 간섭 없이 독립적으로 실행되어야한다
    D(Durabillity) 지속성 : 트랜잭션이 커밋되면, 그 결과는 시스템이 종료되고 영구적으로 보존되어야한다
*/