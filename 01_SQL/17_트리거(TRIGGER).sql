/*
    ...
    *적용대상(어디에 적용하는가)
    - 문장 트리거 : 이벤트가 발생한 SQL문에 대해 딱 한 번만 트리거 실행(FOR EACH ROW 없음)
    - 행 트리거 : 이벤트가 적용된 각 행마다 실행(FOR EACH ROW 필요)
    *행트리거에서는 변경 전/후 데이터를 참조 가능
        - :OLD = 기존행데이터
        - :NEW = 새로 들어오는 데이터
...
VALUES(301, '조치호', '010708-3000000', 'J7');

-- 입출고에 대한 데이터 기록(INSERT)될 때마다 해당상품에 대한 재고 수량을 매번 수정(UPDATE)
-- 상품테이블(재고), 입출고기록테이블

-- 1. 상품에대한 데이터를 보관할 상품 테이블(TB_PORDUCT)
...
-- TB_PRODUCT테이블에 INSERT이벤트 발생시 
-- TB_PRODETAIL 테이블에 매번 자동으로 재고수량을 UPDATE하는 트리거
