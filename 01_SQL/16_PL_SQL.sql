/*
    <PL/SQL>
    오라클DBMS에서 사용하는 절차적 SQL 확장 언어이다
    일반 SQL의 단점을 보완하여 변수의 정의, 조건(IF), 반복(FOR, WHILE)등을 지원하여 다수의 SQL문을 한번에 실행살 수 있음
    PL/SQL은 블록구조를 사용함
    
    PL/SQL 구조
    DECLARE     -- (선택)선언부 : 변수, 상수 등을 선언
...
/*
    ...
    각 자료형 EMPLOYEE(EMP_ID, EMP_NAME, JOB_CODE, SALARY), DEPARTMENT(DEPT_TITLE)을 참조하여
...
-- BONUS를 받지 않는 사원은 보너스를 지급받지 않는 사원입니다
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    JCODE EMPLOYEE.JOB_CODE%TYPE;
...
-- 래퍼런스 변수 (EID, ENAME, DTITLE, NCODE)를 생성하고 각 테이블에서 EMP_ID, EMP_NAME, DEPT_TITLE, NATIONAL_CODE를 참조래하 일반타입 변수 TMAM을 문자열 타입으로 생성하고 사용자가 입력한 사원정보를 가져와서 사번, 이름, 부서명, 근무국가코트 조회후에 각 변수에 대입
-- NCODE의 값이 KO일 경우 TEAM 국내팀 그 외는 TEAM=해외팀 대입
-- 사번, 이름, 부서명, 소속(TEAM)출력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    DTITLE DEPARTMENT.DEPT_TITLE%TYPE;
...
-- I에 1부터 10을 부여해서 각각 반복문 내의 코드를 실행함
BEGIN
    FOR I IN 1..10
    LOOP
        DBMS_OUTPUT.PUT_LINE(I);
...
END;
/
-- I에 10부터 1인 역순으로 부여해서 각각 반복문 내의 코드를 실행함
BEGIN
