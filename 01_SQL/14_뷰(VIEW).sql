...
/*
    DML명령이 사용이 불가한 경우
    1) 뷰에 없는 컬럼을 조작
    2) 뷰에 정의되어있지 않은 컬럼 중에 NOT NULL 제약조건이 있는 컬럼이 있는 경우
    3) 산술/함수식을 사용한 경우
    4) 그룹함수나 GROUP BY절을 포함한 경우
    5) DISTINST 구문이 포함된 경우
...
/*
    ...
    > NOFORCE : 서브쿼리에 기술된 테이블이 존재할 때만 뷰 생성 가능(기본 값
    
    > WITH CHECK OPTION : 뷰의 WHERE조건을 강제하여, 조건에 맞는 데이터만 삽입/수정 가능하게 함
    > WITH READ ONLY : 뷰에 대해서 조회만 가능하게 설정
*/
...
CREATE OR REPLACE VIEW VW_EMP
AS(SELECT * FROM EMPLOYEE WHERE BONUS IS NOT NULL)
WITH READ ONLY;
